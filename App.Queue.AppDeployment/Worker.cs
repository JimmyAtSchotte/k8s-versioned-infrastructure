...
public class Worker : BackgroundService
{
    ...
    public Worker(IOptions<WorkerOptions> options, ILogger<Worker> logger)
    {
        _queue = options.Value.Queue;
        _logger = logger;
        _factory = new ConnectionFactory()
        {
            HostName = options.Value.QueueHost,
    ...
    private async Task ConsumerOnReceived(BasicDeliverEventArgs args, CancellationToken cancellationToken)
    {
        try
        {
           ...
           _logger.LogInformation("Received: {message}", message);
            ...
            _logger.LogInformation("Action: {messageAction} (version: {messageVersion})", queueMessage.Action, queueMessage.Version);
